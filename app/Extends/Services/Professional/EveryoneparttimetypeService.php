...
namespace App\Extends\Services\Professional;
...
class EveryoneparttimetypeService
{
    ...
    public function updateExtendField($pid):bool{
        ...
        $parentInfo = Everyoneparttimetype::bFind($pid);
        if(!$parentInfo) return false;

        $parent_name = trim($parentInfo['parent_name'].','.$parentInfo['name'],',');

        $levels = trim($parentInfo['levels'].','.$parentInfo['id'],',');
        $childList = DB::table(Everyoneparttimetype::tableName())->where('parent_id',$pid)->get();
    ...
    {
        $list = DB::table(Country::tableName())->select(['id', 'name'])->orderBy('listsort','asc')->orderBy('id','asc')->get();
